...
| Download | [Releases](https://github.com/neryosx/workbay/releases), CurseForge, Modrinth |
...
| `#workbay:host_denied` | An explicit deny, reported to the player as "the pack forbade it". Beds, doors, trial spawners and vaults ship on it. |
| `#workbay:host_denied_types` | The same, on a **block entity type**, so one entry covers every tier of a machine at once. |

Workbay also honours NeoForge's own `#c:relocation_not_supported`, and reports that case as "cannot be moved" rather than as a pack decision - two different problems, two different things for the player to do next.
...
**Beyond the tags there is less than you might assume.** There is no multiblock check and no rotation check. The only code-level rules are: a block with no block entity is refused, a block with three or more of the boolean `north/east/south/west/up/down` properties is refused as a connecting block, a Workbay is refused inside a Workbay, and everything else is allowed. If a machine in your pack should not be hostable, tag it - nothing else will catch it.
...
The whole tree is deliberately cheap and early - see **[[Recipes and Upgrades]]**. If your pack is a progression pack, the two recipes worth gating are the **Workbay** itself and the **Housing**, because everything else hangs off those two.

## Performance

A hosted machine is the same block entity ticking at the same rate it would on the floor; Workbay only relocates it. Measured overhead is about **0.05 ms per tick per busy network**, and a network holds at most 64 links.

On chunks: the bay column is mirrored only while the Workbay's own chunk is already loaded, so it costs no chunk a player is not already paying for, and `roomsLoadWithWorkbay` adds one per room on the same terms. Only an Anchor keeps anything loaded while nobody is present, and only while its owner is online plus the grace period.
